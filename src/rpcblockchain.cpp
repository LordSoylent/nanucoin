...
//        return 0; 
    ...
    double dAvg = 0;
    int nWorkdsHandled = 0, nTime = 0;
    
    CBlockIndex* pindex = chainActive.Tip();
    CBlockIndex* pindexPrevWork = NULL;
 
    while (pindex && nWorkdsHandled < nPoWInterval) {
        if (pindex->IsProofOfWork()) {
            if (pindexPrevWork) {
                dAvg += GetDifficulty(pindexPrevWork) * 4294.967296;
                nTime += pindexPrevWork->nTime - pindex->nTime;
                nWorkdsHandled++;
            } 
...
Value getrawmempool(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "    \"fee\" : n,              (numeric) transaction fee in nanucoin\n"
...
Value gettxout(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw runtime_error(
            ...
            "     \"addresses\" : [          (array of string) array of nanucoin addresses\n"
            "     \"nanucoinaddress\"   	 	(string) nanucoin address\n"
            "        ,...\n"
            "     ]\n"
            "  },\n"
