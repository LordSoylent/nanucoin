...
Value mnbudget(const Array& params, bool fHelp)
{
    ...
    if (fHelp ||
        ...
        throw runtime_error(
            ...
            "  vote-many          - Vote on a NanuCoin initiative\n"
            "  vote-alias         - Vote on a NanuCoin initiative\n"
            "  vote               - Vote on a NanuCoin initiative/budget\n"
    ...
    if (strCommand == "prepare") {
        ...
        if (params.size() != 7)
            throw runtime_error("Correct usage is 'mnbudget prepare proposal-name url payment_count block_start nanucoin_address monthly_payment_nanucoin'");
        ...
        if (!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid NanuCoin address");

        // Parse NanuCoin address
    ...
    if (strCommand == "submit") {
        ...
        if (params.size() != 8)
            throw runtime_error("Correct usage is 'mnbudget submit proposal-name url payment_count block_start nanucoin_address monthly_payment_nanucoin fee_tx'");
